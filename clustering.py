...
if st.sidebar.checkbox("RAW DATA"):
    ...
    RawData = pd.read_csv('Mall_Customers2.csv')
    st.write(RawData)
    if st.button("ADD CUSTOMER"):
        buff, col, buff2 = st.beta_columns([3,2,1])
        User_customer = buff.text_input("Customer_ID")
        User_age = buff.text_input("Age")
        User_gender = buff.text_input("Gender")
        User_Income = buff.text_input("Annnual_Income")
        User_amount_spent = buff.text_input("Amount_Spent")
        User_purchased_date = buff.text_input("Purchased_Date")

        List=[User_customer,User_gender,User_age,User_Income,User_purchased_date,User_amount_spent]
...
# Arguments (x = value, p = recency, monetary_value, frequency, k = quartiles dict)
    ...
    st.write(rfm_segmentation['RFMScore'])

    df = pd.read_csv("Mall_Customers.csv")
    df["RFMScore"] = rfm_segmentation['RFMScore']
    df["RFMScore"].dtype
    df = df.fillna(0)
    df.RFMScore.astype('int64')
    df["RFMScore"].dtype
...
if st.sidebar.checkbox("K-Mean Clustering"):
    ...
    kmeans = kmeans.fit(customer[['Spending_Score','Income']].astype(int))
...
if st.sidebar.checkbox("Hierarchical Clustering "):
